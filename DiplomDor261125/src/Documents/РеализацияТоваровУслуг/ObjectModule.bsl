...
Процедура ВыполнитьАвтозаполнение() Экспорт
	...
	РаботаСпеца = Константы.ВКМ_НоменклатураРаботыСпециалиста.Получить();
	
	Если Не ЗначениеЗаполнено(АбонПлата) ИЛИ Не ЗначениеЗаполнено(РаботаСпеца) Тогда
	...
	АбонентскаяПлатаДогвор = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Договор, "ВКМ_АбонентскаяПлата");
		
    Услуги.Очистить();
    
    Если Не АбонентскаяПлатаДогвор = 0  Тогда
	   	ТабЧастьУслуги = Услуги.Добавить();
		ТабЧастьУслуги.Номенклатура = АбонПлата;
		ТабЧастьУслуги.Количество = 1;
		ТабЧастьУслуги.Цена = АбонентскаяПлатаДогвор;
		ТабЧастьУслуги.Сумма = АбонентскаяПлатаДогвор;   
    КонецЕсли; 
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |ИЗ
		               |	РегистрНакопления.ВКМ_ВыполненныеКлиентуРаботы.Обороты(&ДатаНачало, &ДатаКонец, Месяц, Договор = &Договор) КАК ВыполненныеКлиентуРаботыОбороты";
		...
		Пока Выборка.Следующий() Цикл
			...
			Строка.Номенклатура = РаботаСпеца;
			Строка.Количество = Выборка.КоличествоЧасов; 
			Строка.Цена = Выборка.КоличествоЧасов/Выборка.СуммаКОплате;
